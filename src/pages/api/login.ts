...
const login = async (req: NextApiRequest, res: NextApiResponse<any>) => {
  const userLoginFunction = async (credential: Credential): Promise<any> => {
    const { data }: PostgrestSingleResponse<any> = await supabase
      .from('userTable')
      .select('*')
      .eq('userLogin', credential.userLogin.toLowerCase())
      .single()
    let user: Response = {}
  ...
  const { accessToken, user }: { accessToken: string; user: Response } =
    await userLoginFunction(credential)
  if (accessToken !== '') {
    res.status(200).json({ isLoggedIn: true, ...user, accessToken })
  } else {
